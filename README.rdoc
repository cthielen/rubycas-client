...
{omniauth-cas}[https://github.com/dlindahl/omniauth-cas]
...
1. The Rails session store must be set to ActiveRecord: <tt>config.action_controller.session_store = :active_record_store</tt>. Note in Rails 4.0 and 4.1, you must add 'activerecord-session_store' to your Gemfile to support Active Record-backed session stores.
2. (The following does not seem to apply to Rails 4.1, perhaps 4.0 as well.) The server must be able to read and write to Rails.root/tmp/sessions. If you are in a clustered environment,
...
4. You must add a column to your 'sessions' table (a requirement of using Active Record Session Store) called service_ticket (string).
5. Finally, you must add <tt>:enable_single_sign_out => true</tt> and <tt>:ticket_store => :active_record_ticket_store</tt> to your CAS client config (a similar option must be
